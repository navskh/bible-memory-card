    ...
    >
      <div className="p-6 h-full flex flex-col">
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-indigo-800">DEP242 암송 카드</h2>
        ...
        <div className="mb-6 p-4 bg-indigo-50 rounded-lg">
          ...
          <div className="w-full bg-indigo-200 rounded-full h-2">
            <div
              ...
              style={{ width: `${(checkedCards.size / memoryCards.length) * 100}%` }}
            ></div>
          </div>
        </div>
          ...
          {Object.entries(groupedCards).map(([day, cards]) => (
            <div key={day} className="space-y-2">
              ...
              <div className="sticky top-0 bg-white/80 backdrop-blur-sm py-2 border-b border-gray-200">
                <h3 className="font-semibold text-indigo-700 text-sm">{day}일차</h3>
              </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      >
                        {checkedCards.has(card.id) && <Check className="w-2 h-2" />}
                      </button>

                      {/* 카드 정보 */}
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold text-gray-800 text-xs mb-1">{card.subTitle}</div>
                        {card.subSubTitle && (
                          <div className="text-xs text-gray-600 mb-1">{card.subSubTitle}</div>
                        )}
                        <div className="text-xs text-indigo-600 mb-1">({card.reference})</div>
                        <div className="text-xs text-gray-600 line-clamp-2">{card.verse.substring(0, 40)}...</div>
                      </div>
                    </div>
                  </div>
                )
